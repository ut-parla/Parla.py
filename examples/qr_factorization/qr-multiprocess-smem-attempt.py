...
import os
os.environ["OMP_NUM_THREADS"] = "6" # export OMP_NUM_THREADS=6
...
def shared_ndarr(dima, dimb):
    shared_arr = Array(c_double, dima * dimb)
    return np.frombuffer(shared_arr.get_obj()).reshape(dima, dimb)
...
def tsqr_blocked_multi(A, block_size, workers):
    if COLS > block_size:
        ...
        exit(1)
    A_blocked, nblocks = make_blocked(A, block_size) # A_blocked is a list of ndarrays in shared memory

    # We need our q outputs to go straight to shared memory to avoid copying. Create the shared memory here
    Q1 = [shared_ndarr(block_size, COLS) for i in range(nblocks)]
    R1 = []
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers = workers) as executor:
        ...
        Q2 = shared_ndarr(R1.shape[0], R1.shape[1])
        a = Q2
...
if __name__ == "__main__":
    ...
    # Original matrix, stored in shared memory
    """
    A = np.random.rand(ROWS, COLS)
    """
    A = shared_ndarr(ROWS, COLS)
    A[:] = np.random.uniform()
    
    # Multi-process blocked
    start = time.time()
