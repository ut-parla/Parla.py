...
def cholesky_blocked_inplace(shape, num_gpus):
  ...
  # Define task spaces
  gemm1d = TaskSpace("gemm1d")        # Inter-block GEMM
  gemm1 = TaskSpace("gemm1")        # Inter-block GEMM
  subcholeskyd = TaskSpace("subcholeskyd")  # Cholesky on block
  subcholesky = TaskSpace("subcholesky")  # Cholesky on block
  gemm2d = TaskSpace("gemm2d")        # Inter-block GEMM
  gemm2 = TaskSpace("gemm2")        # Inter-block GEMM
  solved = TaskSpace("solved")        # Triangular solve
  solve = TaskSpace("solve")        # Triangular solve

  for j in range(shape[0]):
    for k in range(j):
      # Inter - block GEMM
      @spawn(gemm1d[j, k], [solve[j, k]], placement=[gpu(j%num_gpus)])
      def t1d():
        move_gpu_memory(j, j, num_gpus)
        move_gpu_memory(j, k, num_gpus)
      @spawn(gemm1[j, k], [gemm1d[j, k]], placement=[gpu(j%num_gpus)])
      def t1():
        out = get_raw_gpu_memory(j, j, num_gpus)
        rhs = get_raw_gpu_memory(j, k, num_gpus)
        out = update(rhs, rhs, out)
        set_gpu_memory_from_gpu(j, j, num_gpus, out)

    # Cholesky on block
    @spawn(subcholeskyd[j], [gemm1[j, 0:j]], placement=[gpu(j%num_gpus)])
    def t2d():
      move_gpu_memory(j, j, num_gpus) 
    @spawn(subcholesky[j], [subcholeskyd[j]], placement=[gpu(j%num_gpus)])
    def t2():
      dblock = get_raw_gpu_memory(j, j, num_gpus) 
    ...
    for i in range(j+1, shape[0]):
      for k in range(j):
        # Inter - block GEMM
        @spawn(gemm2d[i, j, k], [solve[j, k], solve[i, k]], placement=[gpu(i%num_gpus)])
        def t3d():
          move_gpu_memory(i, j, num_gpus)
          move_gpu_memory(i, k, num_gpus)
          move_gpu_memory(j, k, num_gpus)
        @spawn(gemm2[i, j, k], [gemm2d[i, j, k]], placement=[gpu(i%num_gpus)])
        def t3():
          out = get_raw_gpu_memory(i, j, num_gpus)
          rhs1 = get_raw_gpu_memory(i, k, num_gpus)
          rhs2 = get_raw_gpu_memory(j, k, num_gpus)
          out = update(rhs1, rhs2, out)
          set_gpu_memory_from_gpu(i, j, num_gpus, out)

      # Triangular solve
      @spawn(solved[i, j], [gemm2[i, j, 0:j], subcholesky[j]], placement=[gpu(i%num_gpus)])
      def t4d():
        move_gpu_memory(j, j, num_gpus)
        move_gpu_memory(i, j, num_gpus)
      @spawn(solve[i, j], [solved[i, j]], placement=[gpu(i%num_gpus)])
      def t4():
        factor = get_raw_gpu_memory(j, j, num_gpus)
        panel  = get_raw_gpu_memory(i, j, num_gpus)
...
def move_gpu_memory(i:int, j:int, num_gpus:int):
  dev_id   = i % num_gpus
  local_id = i // num_gpus
  src = gpu_arrs[dev_id][local_id][j]
  src = clone_here(src)

def get_raw_gpu_memory(i:int, j:int, num_gpus:int):
  dev_id   = i % num_gpus
  local_id = i // num_gpus
  src = gpu_arrs[dev_id][local_id][j]
  return src
