...
parser.add_argument('-nthreads', type=int, default=1, help="Number of workers to use")
#Number of devices to use
parser.add_argument('-ithreads', type=int, default=1, help="Internal Threads")
args = parser.parse_args()

import mkl
load = 1.0/args.nthreads

mkl.set_num_threads(args.ithreads)
os.environ["NUMEXPR_NUM_THREADS"] = str(args.ithreads)
os.environ["OMP_NUM_THREADS"] = str(args.ithreads)
os.environ["OPENBLAS_NUM_THREADS"] = str(args.ithreads)
os.environ["VECLIB_MAXIMUM_THREADS"] = str(args.ithreads)
...
def main():
    ...
    async def main_task():
        ...
        blocks = args.nthreads
        block_size = n // args.nthreads
        h_ordr = 'C'
        d_ordr = 'F'
        print("BlockSize: ", block_size, "Workers: ", blocks)
        ...
        for repetition in range(repetitions):
            ...
            for i in range(blocks):
                for j in range(blocks):
                    ...
                    @spawn(matmul[i, j], placement = cpu, memory=memsize, vcus=load)
                    def matmul_task():
                        ...
                        if verbose:
                            print(f"+({i}, {j}): ", a.shape, b.shape, c.shape, flush=True)
