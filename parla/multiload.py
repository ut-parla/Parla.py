...
"""
def import_override_old(name, *args, **kwargs):
    # TODO: we probably don't want to multiload standard library functions either,
    # but it's harder to check for those, so for now just don't multiload builtins.
    if wrap_imports:
        full_name = get_full_name(name, *args, **kwargs)
        print("names:", full_name, name, args[2], args[3])
        if full_name in in_progress:
            ret = builtin_import(name, *args, **kwargs)
            print(full_name, "returning because in progress")
            return ret
        was_loaded = full_name in sys.modules
        with multiload_in_progress(full_name):
            module = builtin_import(name, *args, **kwargs)
        if is_exempt(full_name, module):
            print(full_name, "returning because exempt")
            return module
        # TODO: Is the first load ever one where the module isn't
        # what's being returned? If so, some of the later code may
        # be redundant.
        desired_submodule = module
        accesses = name.split(".")[1:]
        if not args[2]:
            #accesses = name.split(".")[1:]
            for submodule_name in accesses:
                desired_submodule = getattr(desired_submodule, submodule_name)
        if name == "multiload_test_module.absolute":
            global count
            #if count:
            #    print(id(desired_submodule))
            #    print(id(sys.modules[full_name]))
            #    print(dir(desired_submodule))
            #    print("name:", desired_submodule.__name__)
            #    print(desired_submodule._parla_load_id)
            #    print(object.__dir__(desired_submodule))
            #    print(int(current_context))
            count += 1
        if getattr(desired_submodule, "_parla_forwarding_module", False):
            print(full_name, "returning previously constructed forwarding module (or its containing module).")
            return module
        if was_loaded:
            #print(module.__name__)
            #print(desired_submodule.__name__)
            #print(name, args[2:])
            #print(desired_submodule._parla_forwarding_module)
            #print(name, args, kwargs)
            print(full_name, id(module), module.__name__, module.absolute.__name__)
            print("desired submodule name:", desired_submodule.__name__)
            print(dir(desired_submodule))
            print("desired submodule load id:", desired_submodule._parla_load_id)
            print("desired submodule id:", id(desired_submodule))
            print("sys.modules entry:", id(sys.modules[full_name]), sys.modules[full_name].__name__)
            print("module's name", module.__name__)
            raise ImportError(
                "Trying to multiload module {} which has already "
                "been imported without multiloading.".format(full_name))
        base_modules = []
        needs_parent_attribute_update = accesses and not args[2]
        if accesses:
            if args[2]:
                parent_module = module
                for submodule_name in accesses[:-1]:
                    parent_module = getattr(parent_module, submodule_name)
            else:
                parent_module_name = ".".join(full_name.split(".")[:-1])
                parent_module = sys.modules[parent_module_name]
            end_name = name.split(".")[-1]
        #if needs_parent_attribute_update:
        #    parent_module = module
        #    for submodule_name in accesses[:-1]:
        #        parent_module = getattr(parent_module, submodule_name)
        #    end_name = name.split(".")[-1]
                ...
                if i:
                    with multiload_in_progress(full_name):
                        ...
                        if not args[2]:
                            for submodule_name in accesses:
                                desired_submodule = getattr(desired_submodule, submodule_name)
                #if hasattr(desired_submodule, "_parla_load_id"):
                #    print(full_name, needs_parent_attribute_update)
                #    print(i, desired_submodule._parla_load_id)
                assert not hasattr(desired_submodule, "_parla_load_id")
                desired_submodule._parla_load_id = i
                #print(id(desired_submodule))
                base_modules.append(desired_submodule)
                if needs_parent_attribute_update:
                    delattr(parent_module, end_name)
                del sys.modules[full_name]
        forwarding_module = forward_module(base_modules)
        #forwarding_module._parla_import_name = name
        #forwarding_module._parla_import_args = args
        #forwarding_module._parla_import_kwargs = kwargs
        sys.modules[full_name] = forwarding_module
        assert getattr(forwarding_module, "_parla_forwarding_module", False)
        if needs_parent_attribute_update:
            setattr(parent_module, end_name, forwarding_module)
        print(full_name, name, "returning forwarding module:", id(forwarding_module))
        #print(dir(forwarding_module))
        #print(forwarding_module.a)
        #print(id(forwarding_module))
        #print(id(sys.modules[full_name]))
        if not args[2]:
            return module
        return forwarding_module
