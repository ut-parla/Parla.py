...
class ResourcePool:
    ...
    def track_parray(self, parray):
        logger.debug(
            f"[ResourcePool] Tracking parray with ID %d in these locations:", id(parray))
    ...
    def untrack_parray(self, parray):
        logger.debug(
            f"[ResourcePool] Untracking parray with ID %d from these locations:", id(parray))
        ...
        for device, parray_exists in self._managed_parrays[id(parray)].items():
            if parray_exists:
                ...
                self.deallocate_resources(
                    device, {'memory': parray_tracker.nbytes})
                logger.debug(f"[ResourcePool]   - %r", device)
    ...
    def add_parray_to_device(self, parray, device):
        logger.debug(
            f"[ResourcePool] Adding parray with ID %d to device %r", id(parray), device)
    ...
    def remove_parray_from_device(self, parray, device):
        logger.debug(
            f"[ResourcePool] Removing parray with ID %d from device %r", id(parray), device)
    ...
    def parray_is_on_device(self, parray, device):
        ...
        with self._monitor:
            ret_bool = (id(parray) in self._managed_parrays) and (
                self._managed_parrays[id(parray)].locations[device])
            logger.debug(
...
# self._datamove_task_from_dev_queues = {dev: deque() for dev in self._available_resources.get_resources()}
    ...
    def _construct_datamove_task(self, target_data, compute_task: ComputeTask, operand_type: OperandType):
        ...
        taskid = TaskID(str(compute_task.taskid)+"."+str(hex(id(target_data)))+".dmt." +
        ...
        if id(target_data) != target_data_id:
            self._datablock_dict[id(target_data)].append(
                (str(compute_task.taskid), compute_task))
